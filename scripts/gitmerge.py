...
# The script is not mine, I got it from here:
# http://www.jperla.com/blog/post/a-clean-python-shell-script
# It was pretty old, so I updated it to use argparse and 
# python3.
...
def main():
    parser = argparse.ArgumentParser(
        description="Git merges current branch with master, or vice versa.")

    parser.add_argument("-m", "--merge-master", dest="merge_master",
                      action="store_true", default=False,
                      help="Merges the latest master to the current branch.")

    parser.add_argument("-B", "--merge-branch", dest="merge_branch",
                      action="store_true", default=False,
                      help="Merges the current branch into master; forces -m")

    args = parser.parse_args()

    if not args.merge_master and not args.merge_branch:
    ...
    if args.merge_master:
        ...
        match = re.search('On branch ([^\s]*)', str(output, 'utf-8'))
        branch = None
        print(branch)
        if match is None:
            raise Exception('Could not get status.')
        elif match.group(1) == 'master':
            raise Exception('You must be in the branch you want to merge, not master')
        else:
            branch = match.group(1)
            logging.info('In branch %s' % branch)

        if str(output, 'utf-8').endswith('nothing to commit, working directory clean\n'):
        ...
        output, _ = call_command('git pull')
        if 'up-to-date' not in str(output, 'utf-8'):
            raise Exception('Local copy was not up to date:\n%s' % output)
        else:
            logging.info('Local copy was up to date.')
